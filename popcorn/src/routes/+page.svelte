<script>
  ...
  async function save() {
    ...
    try {
      if (scope === 'local') {
        const item = { ...draft, id: createId(), createdAt: new Date().toISOString() };
  ...
  function changeLanguage(value){$language=value;localStorage.setItem('popcorn:language',value);localStorage.setItem('save:language',value);loadMovies();}
