<script>
  ...
  function editSaved(item,familyItem){draft={...item};scope=familyItem?'family':'local';editorOpen=true}
  function openFolder(folder,isLocal){activeFolder={...folder,isLocal};folderViewerOpen=true}
  function editFolder(folder,isLocal){folderDraft=structuredClone(folder);folderScope=isLocal?'local':'family';folderOpen=true}
