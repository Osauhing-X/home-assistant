...
<div class="shell">
  ...
  <main>
    ...
    {:else if tab === 'saved'}
      ...
      <section class="library"><div class="section-head compact"><h2>Minu nimekiri <span>{local.length}</span></h2><small>AINULT SELLES SEADMES</small></div>{#if local.length}<div class="saved-grid">{#each local as item}<Saved {item} familyItem={false} onremove={() => remove(item,false)} />{/each}</div>{:else}<Empty text="Sinu isiklik nimekiri on tühi." />{/if}</section>
      <section class="library"><div class="section-head compact"><h2>Pere nimekiri <span>{family.events.length}</span></h2><small class:online={ha.connected}>HOME ASSISTANT</small></div>{#if family.events.length}<div class="saved-grid">{#each family.events as item}<Saved {item} familyItem={true} folder={family.folders.find(x=>x.id===item.folderId)} onremove={() => remove(item,true)} />{/each}</div>{:else}<Empty text="Pere nimekiri on veel tühi." />{/if}</section>
    {:else if tab === 'folders'}
