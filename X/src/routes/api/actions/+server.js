...
const ACTIONS = new Set(['start', 'stop', 'restart', 'update', 'install', 'sync-integrations', 'update-integration', 'update-all-integrations', 'scan-repository']);

export async function POST({ request }) {
  const input = await request.json();
  if (!ACTIONS.has(input.action)) error(400, 'Unknown action.');
  if (!['sync-integrations', 'update-integration', 'update-all-integrations', 'scan-repository'].includes(input.action)) {
