<script>
  ...
  async function load({ preserveForm = false } = {}) {
    ...
    const discoveredApp = state.config.repositories.find((item) => item.fullName === (repository || configuredApp?.repository))?.applications?.find((item) => item.id === (configuredApp?.sourceId || id)) || catalog;
  ...
  async function action(type) {
    try {
      error = '';
      if (['install', 'start', 'restart'].includes(type) && missingRequiredEnv.length) {
        tab = 'env';
  ...
  async function save() {
    try {
      error = '';
      app.env = parseEnv();
      if (configured) await api('apps', { method: 'PUT', body: JSON.stringify({ ...app, saveOnly: true }) });
      else await api('apps', { method: 'POST', body: JSON.stringify({ ...app, pluginPath: app.pluginPath || app.path, configureOnly: true }) });
      queuedAction = '';
      message = 'Configuration saved. Restart the application to apply runtime changes.';
      await load();
    } catch (reason) { error = reason.message; }
  }
  ...
  async function installAnother() {
    try {
      error = '';
      const created = await api('apps', { method: 'POST', body: JSON.stringify({ ...app, id: app.sourceId || app.id, sourceId: app.sourceId || app.id, env: {}, configureOnly: true }) });
      location.href = `${base}/application?id=${encodeURIComponent(created.app.id)}`;
    } catch (reason) { error = reason.message; }
  }
  ...
  {#if app}
    ...
    <section class="panel">
      {#if tab === 'overview'}
        ...
        {#if configured}<div class="overview-action"><button class="delete-app" on:click={removeApplication}>Delete application</button>{#if status.installed}<button on:click={installAnother}>Install another instance</button>{/if}</div>{/if}
      {:else if tab === 'config'}
        <div class="facts"><label>Application ID<input disabled value={app.id} /></label><label>Repository<input disabled value={app.repository || repository || ''} /></label><label>Path<input bind:value={app.pluginPath} placeholder={app.path || '.'} /></label><label>Port assigned by X Platform<input disabled value={app.port} /></label><label>Update policy<select bind:value={app.updatePolicy}><option value="manual">Manual approval</option><option value="automatic">Automatic update</option></select></label><label>Install command<input bind:value={app.install} /></label><label>Build command<input bind:value={app.build} /></label><label>Start command<input bind:value={app.start} /></label><label>Logo path<input bind:value={app.logo} /></label><label>Background path<input bind:value={app.background} /></label></div>
        ...
        <button class="primary save" on:click={save}>Save configuration</button><small class="restart-note">Restart the application to apply saved runtime changes.</small>
      {:else if tab === 'env' && app.envSchema?.length}
        ...
        <div class="save-row"><button class="primary" on:click={save}>Save environment</button><small>Restart the application for environment changes to take effect.</small></div>
      {:else if tab === 'logs'}
...
<style>
  ...
  .conflict{padding:10px;border:1px solid #69343d;border-radius:6px;background:#32171c;color:#f2a0aa!important}.env-schema{display:grid;gap:9px;margin-bottom:14px}.env-schema article{display:grid;gap:9px;padding:11px;border:1px solid #29313b;border-radius:6px}.env-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.env-schema code{font-weight:800}.env-schema b{padding:2px 5px;border-radius:99px;background:#332b10;color:#da3;font-size:9px;text-transform:uppercase}.env-schema span{color:#9ba5b1}.env-schema small{width:100%;color:#707b89}.env-schema textarea{width:100%;max-width:none}.save-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.save-row small,.restart-note{display:block;color:#7f8a98}.restart-note{margin-top:8px}
  ...
  .overview-action{display:flex;gap:8px;flex-wrap:wrap;margin-top:24px;padding-top:18px;border-top:1px solid #29313b}.delete-app{margin-left:0}
