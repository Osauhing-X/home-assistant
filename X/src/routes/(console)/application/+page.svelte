<script>
  ...
  let message = '', error = '', envText = '', envValues = {}, tab = 'overview';
  ...
  async function load() {
    ...
    const schemaNames = new Set((app?.envSchema || []).map((item) => item.name));
    envValues = Object.fromEntries((app?.envSchema || []).map((item) => [item.name, configuredEnv[item.name] || '']));
    envText = Object.entries(configuredEnv).filter(([key]) => !schemaNames.has(key)).map(([key, value]) => `${key}=${value}`).join('\n');
  ...
  {#if app}
    ...
    <section class="panel">
      ...
      {:else if tab === 'env'}
        ...
        {#if app.envSchema?.length}<div class="env-schema">{#each app.envSchema as variable}<article><div class="env-meta"><code>{variable.name}</code>{#if variable.required}<b>Required</b>{/if}<span>{variable.description || variable.label}</span>{#if variable.example}<small>Example: {variable.example}</small>{/if}</div><textarea rows="2" bind:value={envValues[variable.name]} placeholder={variable.label || variable.name}></textarea></article>{/each}</div>{/if}
        <label class="additional-env">Additional variables <small>Optional, one <code>NAME=value</code> per line.</small><textarea rows="5" bind:value={envText} placeholder="NAME=value"></textarea></label>
...
<style>
  ...
  .conflict{padding:10px;border:1px solid #69343d;border-radius:6px;background:#32171c;color:#f2a0aa!important}.env-schema{display:grid;gap:9px;margin-bottom:14px}.env-schema article{display:grid;gap:9px;padding:11px;border:1px solid #29313b;border-radius:6px}.env-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.env-schema code{font-weight:800}.env-schema b{padding:2px 5px;border-radius:99px;background:#332b10;color:#da3;font-size:9px;text-transform:uppercase}.env-schema span{color:#9ba5b1}.env-schema small{width:100%;color:#707b89}.env-schema textarea{width:100%;max-width:none}.additional-env{display:grid;gap:7px;max-width:900px;color:#909ba8}.additional-env small{color:#707b89}
</style>
