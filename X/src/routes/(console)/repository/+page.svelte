...
{#if repo}
  ...
  <h2>Applications</h2>
  <table><thead><tr><th>Name</th><th>Version</th><th>Path</th></tr></thead><tbody>{#each repo.applications || [] as item}{@const version = applicationVersion(item)}<tr on:click={() => location.href = `${base}/application?id=${encodeURIComponent(item.id)}&repository=${encodeURIComponent(repo.fullName)}`}><td>{item.name}</td><td><b>{version.current}</b>{#if version.available && version.available !== version.current}<span class="version-arrow">➜</span><b class="new-version">{version.available}</b>{/if}</td><td>{item.path}</td></tr>{:else}<tr><td colspan="3">None detected. Add x_config.json to describe applications.</td></tr>{/each}</tbody></table>
{/if}
