...
const defaults={discoveryPort:3702,onvifPort:8091,autoDiscovery:true,fragmentMs:100,gop:5,buffering:false};
...
export async function saveSettings(value){const clean={discoveryPort:Math.min(65535,Math.max(1,Number(value.discoveryPort)||3702)),onvifPort:Math.min(65535,Math.max(1024,Number(value.onvifPort)||8091)),autoDiscovery:value.autoDiscovery!==false,fragmentMs:Math.min(2000,Math.max(50,Number(value.fragmentMs)||100)),gop:Math.min(300,Math.max(1,Number(value.gop)||5)),buffering:Boolean(value.buffering)};await mkdir(dir,{recursive:true});const tmp=`${settingsFile}.${process.pid}.tmp`;await writeFile(tmp,JSON.stringify(clean,null,2));await rename(tmp,settingsFile);return clean}
