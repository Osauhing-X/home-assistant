...
export async function setCameraDiscovery(id,enabled){const all=await cameras(),camera=all.find(item=>item.id===id);if(!camera||(!enabled&&camera.discoveryMode==='off'))return false;camera.discoveryMode=enabled?'on_connect':'off';camera.discoveryEnabled=enabled;camera.discoveryUntil=0;await save(all);return true}
