...
export async function POST({params,request,url}){
  ...
  if(body.includes('GetStreamUri')){const token=body.match(/<(?:\w+:)?ProfileToken>([^<]+)/)?.[1]||'prof0',uri=token==='prof1'&&camera.lq?camera.lq:camera.hq;void logStreamDiagnostics(camera,uri,token);if(camera.discoveryMode==='on_connect'&&await setCameraDiscovery(camera.id,false))await announceBye(camera).catch(error=>console.warn('[WS-Discovery Bye]',error.message))}
