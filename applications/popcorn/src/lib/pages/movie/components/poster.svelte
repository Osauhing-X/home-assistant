...
{#if object&&path}<a class="poster-card" href={`${$page.data.base}/${$page.params.lang||$language}/${type}/${object.id}${$page.url.searchParams.get('language')?`?language=${$page.url.searchParams.get('language')}`:''}`} aria-label={object.title||object.name||$t('View')}><img src={`https://image.tmdb.org/t/p/w500${path}`} alt={object.title||object.name||''} loading="lazy">{#if liked}<span class="liked">♥</span>{/if}</a>{/if}
