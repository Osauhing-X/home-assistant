...
<section id="data" class="grid" full="800">
  ...
  <header>
    ...
    {#if data?.birthday}<span title={$t("Tagline")}>{data.birthday}</span>{/if}
    {#if data?.known_for_department} <span> {data.known_for_department}</span> {/if}
  </header>

      <div class="flex gap" style="gap: 20px">
        <img style="--height:325px" src={data.profile_path ? `https://image.tmdb.org/t/p/w500${data.profile_path}` : ""} alt={$t("poster")}>
        <p>{data.biography}</p>
      </div>
</section>
...
{#if data.movie?.cast?.length > 0}
  <section>
    <h2>{$t("Movie (")}{data.movie?.cast?.length})</h2>
...
{#if data.tv?.cast?.length > 0}
  <section>
    <h2>{$t("Tv (")}{data.tv?.cast?.length})</h2>
    <div class="grid gap trending">
