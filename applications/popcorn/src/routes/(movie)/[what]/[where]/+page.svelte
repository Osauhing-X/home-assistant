<script>
 ...
 import { t } from '$lib/assets/translations';
  ...
  let details=request('where_details');
  $: data=$page.data||{};
...
<main class="movie-shell">
  <article class="detail-hero" style={`--backdrop:url('${backdrop||''}')`}>
    <div class="detail-content">
      ...
      <div class="detail-copy"><span class="eyebrow">{$page.params.what==='tv'?$t("Series"):$page.params.what==='person'?$t("Person"):$t("Movie")}</span><h1>{title}</h1><div class="meta-row">{#if data.release?.date}<span>{data.release.date}</span>{/if}{#if data.runtime}<span>{data.runtime}</span>{/if}{#if data.known_for_department}<span>{data.known_for_department}</span>{/if}{#if data.vote_average}<span>★ {data.vote_average.toFixed(1)}</span>{/if}</div>{#if data.tagline}<p><b>{data.tagline}</b></p>{/if}<p>{data.overview||data.biography||''}</p></div>
    </div>
  </article>
  <div class="detail-save"><div class="action-row">{#if data.trailer}<a href={data.trailer} target="_blank" rel="noreferrer">{$t("Watch trailer ↗")}</a>{/if}{#if data.homepage}<a href={data.homepage} target="_blank" rel="noreferrer">{$t("Official website ↗")}</a>{/if}</div><a class="primary" href={`${$page.data.base}/${$page.params.lang||"en"}/?saveId=${data.id}&saveType=${$page.params.what}&saveTitle=${encodeURIComponent(title)}&savePoster=${encodeURIComponent(data.poster_path||data.profile_path||'')}`}>{$t("Save to list")}</a></div>
